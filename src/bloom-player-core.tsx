...
export class BloomPlayerCore extends React.Component<IProps, IPlayerState> {
    ...
    private getRequiredVersionMessage(bookDoc: Document): string {
        ...
        if (
            ...
            if (host) {
                // This might be a bit problematic for localization, but hopefully the localizer can figure out
                // something that's not too bad. Hopefully these messages are rarely seen.
                host = "(" + host + ")";
            }
            const pattern = LocalizationManager.getTranslation(
                "Version.Problem",
                this.props.preferredUiLanguages,
                "This book uses the feature {0} and requires a newer version of the program you are using {host} to read it. Please upgrade to the latest version.",
            );
            return pattern
                .replace("{0}", highestFeature)
                .replace("{host}", host)
                .replace("  ", " "); // in case there are two spaces where (host) was omitted
        }
    }
