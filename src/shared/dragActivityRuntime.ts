...
export function shuffle<T>(
    ...
    // See BL-14446.
    if (array.length < 2) return array; // nothing to do
    const uniqueItems: T[] = [];
    for (let i = 0; i < array.length; ++i) {
        if (areItemsTheSame) {
            let isNewItem = true;
            for (let j = 0; j < uniqueItems.length; ++j) {
                if (areItemsTheSame(uniqueItems[j], array[i])) {
                    isNewItem = false;
                    break;
                }
            }
            if (isNewItem) uniqueItems.push(array[i]);
        } else {
            // This test likely uses object identity.
            if (!uniqueItems.includes(array[i])) uniqueItems.push(array[i]);
        }
    }
    if (uniqueItems.length === 1) {
        return array; // no shuffling will do any good
    }
    ...
    while (changes === 0 && loopCount++ < 10) {
        ...
        // Check that the shuffling has had an actual visible effect.
        for (let i = 0; i < array.length; ++i) {
            if (areItemsTheSame) {
...
// This method can be passed to shuffle for letters embedded in draggable HTML elements.
