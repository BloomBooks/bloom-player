...
DavidAndGoliathPlayerTest.play = async ({ canvasElement }) => {
    ...
    // At the beginning there is only one visible swiper button (Next).
    ...
    await userEvent.click(nextButton.parentElement!);

    // Now there should be 2 visible buttons.
    const buttons = await playerDOM.findAllByRole("button");
    expect(buttons.length).toEqual(2);
    expect(buttons[0].parentElement!.classList).toContain("swiper-button-prev");
